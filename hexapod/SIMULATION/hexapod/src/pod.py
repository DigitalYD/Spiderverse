...
from src.config import (FORWARD, REVERSE, SIM_ACTIVE, POD_Z_HEIGHT, INTERPOLATION_STEPS, 
                        EFFECTOR_ORIGIN_INDEX, NUM_LEGS, STRAIGHT, LEFT, RIGHT, CLOCKWISE, COUNTERCLOCKWISE)
...
class Pod:
    ...
    rotation: int = STRAIGHT                          # Current rotation (clockwise/Counterclockwise)
    ...
    def rotate_hexapod(self, direction):
        self.rotation = direction
        
        if direction == CLOCKWISE:
            # set control points for legs. Clockwise, left side forward points, right side back points.
            for i, leg in enumerate(self.Legs):
                if leg.Name == "LR" or leg.Name == "LM" or leg.Name == "LF":
                    leg.set_walking_control_points()
                else:
                    leg.set_back_control_points()
        elif direction == COUNTERCLOCKWISE:
            # set control points for legs. Counterclockwise, right side forward points, left side back points.
            for i, leg in enumerate(self.Legs):
                if leg.Name == "LR" or leg.Name == "LM" or leg.Name == "LF":
                    leg.set_back_control_points()
                else:
                    leg.set_walking_control_points()
        else: # if direction is none of the above, set it to straight and do nothing.
            self.rotation = STRAIGHT
            for i, leg in enumerate(self.Legs):
                leg.set_walking_control_points()
