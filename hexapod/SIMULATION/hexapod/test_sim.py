...
from src.config import (COXA_ORIGIN_INDEX, FEMUR_ORIGIN_INDEX, TIBIA_ORIGIN_INDEX, EFFECTOR_ORIGIN_INDEX, CLOCKWISE, COUNTERCLOCKWISE, STRAIGHT)
...
while True:
    for frame in range(STEPS):
        ...
        # CHANGE THE INDEXES, Verify movement direction.
        # May need to set it to forward. prior rotation.
        if index == 800:
            hexapod.rotate_hexapod(CLOCKWISE)
            hexapod.start()
        ...
        if index == 1120:
            hexapod.rotate_hexapod(COUNTERCLOCKWISE)
            hexapod.start()
        ...
        if index == 1450:
            hexapod.rotate_hexapod(STRAIGHT)
            hexapod.reverse_direction()
            # hexapod.direction = 1
            index = 110
        # if index == 550:
