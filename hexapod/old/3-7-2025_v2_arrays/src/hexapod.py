...
class Hexapod:
    ...
    def __init__(self, leg_configs, hexapod_configs):
        ...
        self.executor = ThreadPoolExecutor(max_workers=6)  # Adjust worker count as needed
        self.max_speed = 1.0
        self.legs = {}
        self.gait = "tripod"
        self.mode = "standing"
    ...
    def get_active_legs_in_gait(self):
        # Cache values before the loop
        phase_offsets = self.phase_offsets[self.gait]
        gait_sequence = self.tripod_gait_sequence if self.gait == "tripod" else (
            self.wave_gait_sequence if self.gait == "wave" else self.ripple_gait_sequence
        )
        ...
        # Faster previous leg completion check
        sliding_legs = {leg for leg, state in self.leg_states.items() if state == "sliding"}

        active_legs = [
            leg for leg in current_leg_group
            if all(l in sliding_legs for l in phase_offsets if phase_offsets[l] < phase_offsets.get(leg, float('inf')))
        ]
        return active_legs
    ...
    def inverse_kinematics(self):
        ...
        for leg in active_legs:
            ...
            bezier_index[leg] = self.legs[leg].current_bezier_index
            position[leg] = self.legs[leg].get_swing_foot_position()

            footposition[leg] = np.array([
                self.legs[leg].cur_pos.x + self.body_position.x + position[leg][0],
                self.legs[leg].cur_pos.y + self.body_position.y + position[leg][1],
                self.legs[leg].cur_pos.z + self.body_position.z + position[leg][2]
            ])
            ...
            # Body kinemtatics
            # ------------------------
            body.x = footposition[leg][0] + self.body_coxa_offsets[leg].x
            body.y = footposition[leg][1] + self.body_coxa_offsets[leg].y
            body.z = footposition[leg][2]
            
            # calculate position corrections using rotation matrix
            # https://en.wikipedia.org/wiki/Rotation_matrix
            bodyikPosition[leg] = self.compute_body_rotation(rotation, body)

            #print("BodyikPosition: ",bodyikPosition)
        ...
        futures = {
            self.executor.submit(self.legs[leg].inverse_kinematics, bodyikPosition[leg], footposition[leg]): leg
            for leg in active_legs
        }
    ...
    def compute_body_rotation(self, rotation, body):
        """Computes the rotation correction for foot positions."""
        R = self.rot_mat_3d_z(rotation.z) @ self.rot_mat_3d_y(rotation.y) @ self.rot_mat_3d_x(rotation.x)
        return R @ np.array([body.x, body.y, body.z])  # Apply rotation in one efficient matrix operation

    def update_leg_bezier_state(self, leg):
        """Ensures the leg properly progresses through the Bezier curve."""
        bezier_index = self.legs[leg].current_bezier_index
        bezier_map = self.legs[leg].bezier_curve.index_map
        ...
        for key, state in state_transitions.items():
            if bezier_index >= bezier_map[key]:
                self.leg_states[leg] = state
                if key == "return":
                    self.legs[leg].current_bezier_index = 0  # ✅ Reset for next cycle
    ...
    def advance_step_if_ready(self, active_legs):
        """Advances the step only when all active legs have reached 'sliding'."""
        if all(self.leg_states[leg] == "sliding" for leg in active_legs):
            gait_cycle_length = max(self.phase_offsets[self.gait].values()) + 1
            self.step_count = (self.step_count + 1) % gait_cycle_length
            print(f" Advancing Step Count to {self.step_count}")
