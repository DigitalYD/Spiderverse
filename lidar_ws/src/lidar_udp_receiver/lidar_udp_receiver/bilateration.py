...
class BilaterationNode(Node):
    def __init__(self):
        super().__init__('bilateration_node')
        ...
        self.declare_parameter('reference_y', 300.0)  # Default reference y-coordinate in cm
        self.declare_parameter('prefer_positive_y', True)  # Prefer positive y solutions
        ...
        self.get_logger().info(f"Bilateration node started, listening on {self.server_ip}:{self.server_port}")
    ...
    def calculate_position(self, anchor1_pos, anchor2_pos, distance1, distance2):
        """
        Calculate the position of the tag using bilateration from two anchors.
        
        With only two anchors, we get two possible solutions (points on either side of the line 
        connecting the anchors). We choose one based on previous position or preference settings.
        ...
        Returns:
            Tuple (x, y, z) with position coordinates in cm or None if no valid solution
        """
        ...
        if d == 0:
            self.get_logger().warning("Anchors are at the same position, cannot determine location")
            return None
        
        # Check if the distances make sense
        if distance1 + distance2 < d:
            self.get_logger().warning(f"Invalid distances: sum ({distance1 + distance2}) less than anchor separation ({d})")
            return None
        
        # Calculate position using circle intersection formula
        # Based on https://en.wikipedia.org/wiki/Trilateration
        
        # Simplify by assuming anchor1 is at the origin and anchor2 is along the x-axis
        # This makes the math easier, and we can transform back later
        ...
        dist_squared = distance1**2 - x**2
        if dist_squared < 0:
            self.get_logger().warning("No valid intersection found - measurements may be inaccurate")
        ...
        if self.latest_tag_position is not None:
            ...
            # If both solutions have same sign for y or prefer_positive_y is False,
    ...
    def process_incoming_data(self, json_data):
        ...
        try:
            ...
            # Calculate tag position if both distances are available
            if all(distance is not None for distance in [self.distance_from_anchor_1, 
                ...
                tag_position = self.calculate_position(
                    self.anchor1_pos, self.anchor2_pos, 
                    self.distance_from_anchor_1, self.distance_from_anchor_2
                )
    ...
    def publish_position(self):
        ...
        if hasattr(self, 'prev_position') and hasattr(self, 'prev_timestamp'):
            ...
            if dt > 0:
                ...
                velocity_uncertainty = 0.75  # 0.75 m/s uncertainty (higher than trilateration)
    ...
    def destroy_node(self):
        ...
        self.get_logger().info("Shutting down bilateration node...")
        self.running = False
