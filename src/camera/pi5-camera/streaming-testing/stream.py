...
def create_pipeline(device: str, dest_ip: str, dest_port: int, width: int, height: int, 
    ...
    # Create pipeline based on supported formats
    if 'MJPG' in formats:
        # Camera supports MJPG directly
        pipeline = (
            ...
            f"udpsink host={dest_ip} port={dest_port} sync=true async=true buffer-size=65536"
        )
    else:
        # Fall back to raw format which most cameras support
        pipeline = (
            f"gst-launch-1.0 v4l2src device={device} do-timestamp=true ! "
            f"video/x-raw,width={width},height={height},framerate={framerate}/1 ! "
            f"videoconvert ! jpegenc quality={quality} ! "
            f"rtpjpegpay ! application/x-rtp,media=video ! "
            f"rtpjitterbuffer latency={latency} ! "
            f"udpsink host={dest_ip} port={dest_port} sync=true async=true buffer-size=65536"
        )
    ...
    if debug:
        pipeline = pipeline.replace("udpsink", 
                                   f"tee name=t ! queue ! udpsink")
        pipeline += f" t. ! queue ! videoconvert ! autovideosink"
...
def start_streaming(device: str = None, dest_ip: str = DEFAULT_IP, dest_port: int = DEFAULT_PORT,
    ...
    print(f"Starting camera stream from {device} to {dest_ip}:{dest_port}")
    print(f"Using: width={width}, height={height}, framerate={framerate}, quality={quality}, latency={latency}")
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(description='Raspberry Pi Camera Streaming Tool')
