...
class BezierCurve:
    ...
    def bernstein_poly(self, i, n, t):
        """Computes the Bernstein polynomial for Bézier curve calculation."""
    ...
    def get_point(self, index):
        """Retrieve a specific point on the Bézier curve by index."""
        return self.cp[min(index, len(self.cp) - 1)]

    def get_named_point(self, name):
        """Retrieve a point from the Bézier curve using a named control point."""
        if name in self.index_map:
            return self.get_point(self.index_map[name])
        return None

    def curve(self):
        """Return the generated Bézier curve points."""
        if len(self.cp) == 0:
            self.generate()
        return self.cp
