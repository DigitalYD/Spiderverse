...
class Leg:
    ...
    def inverse_kinematics(self, bodyikposition, footpos):
        ...
        footpos[0] = footpos[0] + bodyikposition[0]
        footpos[1] = footpos[1] + bodyikposition[1]
        footpos[2] = footpos[2] + bodyikposition[2]# Ensure correct Z height
        ...
        toepos.x = footpos[0] * np.cos(theta) - footpos[1] * np.sin(theta)
        toepos.y = footpos[0] * np.sin(theta) + footpos[1] * np.cos(theta)
        toepos.z = footpos[2]  # Maintain correct Z height
        ...
        if self.name in ["RF", "RM", "RR"]:
            coxa_rad = np.arctan2(toepos.x, toepos.y)  # ✅ Flip for right legs
        else:
            coxa_rad = -np.arctan2(toepos.x, toepos.y)

        # ✅ Correct rear legs to move with the front legs
        if self.name in ["RR", "LR"]:  
            coxa_rad = -coxa_rad  # ✅ Reverse for rear legs to follow the front legs
