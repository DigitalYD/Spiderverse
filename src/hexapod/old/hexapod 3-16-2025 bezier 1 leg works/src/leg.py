...
class Leg:
    ...
    def set_gait_control_points(self, stride_length:float = 25.0):
        ...
        self.control_points = {
            "start": start,
            "lift": start + np.array([50, -50, -50]),
            "peak": start + np.array([150, -150, -100]),
            "lower": start + np.array([200, -200, -30]),                 # The moment before grounding
...
def test_bezier_curve_movement(leg) -> None:
    ...
    # if leg.bezier_curve.index >= leg.bezier_curve.index_map["grounded"]:
    #     leg.current_leg_phase = "sliding"
    # else:
    #     leg.current_leg_phase = "lifting"
    ...
    # Handle NaN
    # angles.Coxa = 0 if np.isnan(angles.Coxa) else angles.Coxa
    # angles.Femur = 0 if np.isnan(angles.Femur) else angles.Femur
    # angles.Tibia = 0 if np.isnan(angles.Tibia) else angles.Tibia
    ...
    # print("joints: ", leg.Joints)
    # If you have servo objects, uncomment and adjust:
