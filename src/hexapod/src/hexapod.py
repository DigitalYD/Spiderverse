...
from hexapod_configs import hexapod_configs
...
class Hexapod:
    ...
    def move_leg(self, leg, target_position):
        '''
            Move leg based on inverse kinematic angles
        '''
        return self.legs[leg].inverse_kinematics(target_position)
    
    
    def move_joint(self, leg, joint, amount):
        if leg in self.legs:
            self.legs[leg].set_joint_angle(joint, amount)
        else:
